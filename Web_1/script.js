...
/** MedQuiz (Web_2) — meta clinical-beta-url หรือ localhost:3000 ตอน dev */
function getBetaFunctionUrl() {
    const fromMeta = metaConfig('clinical-beta-url');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!supabaseConfigReady) {
        ...
        tip.lang = 'th';
        tip.style.cssText =
            'position:fixed;top:0;left:0;right:0;z-index:99998;padding:12px 16px;background:#5c2d2d;color:#fff;font-size:14px;text-align:center;line-height:1.45;font-family:system-ui,sans-serif;';
        tip.innerHTML =
    ...
    if (btnBeta) {
        btnBeta.addEventListener('click', () => {
            ...
            if (!url) {
                showToast(
                    'Beta function: set meta clinical-beta-url in index.html to your MedQuiz (Web_2) URL.',
                    'error'
                );
                return;
            }
